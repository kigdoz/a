...
// Sử dụng hàm để lấy một ciphersuite ngẫu nhiên và in ra kết quả
const randomTLSCiphersuite = getRandomTLSCiphersuite();
  const firefoxVersion = "119.0.1"; // Ð?t phiên b?n Firefox theo mong mu?n c?a b?n
...
const coloredString = "recommended big proxyfile if target is akamai/fastly".white;
 ...
 const parsedTarget = url.parse(args.target);
    console.clear()
    console.log("Test Script By : @nminh23".rainbow);
...
function getRandomUserAgent() {
    ...
    const version = `${firefoxVersion}`; // Thay đổi version nếu cần
...
// Sử dụng hàm để lấy giá trị sec-ch-ua
...
colors.enable();
  ...
  function runFlooder() {
    ...
    const parsedPort = parsedTarget.protocol == "https:" ? "443" : "80";
	  ...
	  process.stdout.write('lỗi rồi địt mẹ mày\r');
	  interval = 1000;
	}
